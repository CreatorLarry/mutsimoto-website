...
export async function getAdminCompanyContent(): Promise<{ about: AdminAboutPageContent; leadership: LeadershipProfile[] }> {
  ...
  const [aboutResult, leadershipResult] = await Promise.all([
    ...
    supabase.from("leadership_profiles").select("id, full_name, title, biography, message, photo_storage_path, display_order, published, updated_at").order("display_order"),
  ]);
  ...
  return {
    ...
    leadership: await Promise.all(
      ((leadershipResult.data ?? []) as LeadershipRecord[]).map(async (leader) => ({
        id: leader.id,
        fullName: leader.full_name,
        title: leader.title,
        biography: leader.biography,
        message: leader.message,
        photoStoragePath: leader.photo_storage_path,
        photoUrl: await getLeadershipImageUrl(leader.photo_storage_path),
        displayOrder: leader.display_order,
