  ...
  return (
    <>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-5 pb-16 sm:px-8 lg:grid-cols-[1.02fr_0.98fr] lg:px-10 lg:pb-20">
        <div><div className="aspect-square overflow-hidden rounded-lg border border-[#353d43] bg-[#171c20] shadow-[0_18px_50px_rgba(0,0,0,0.38)]"><FilterVisual category={product.category} imageSrc={product.image} /></div><div className="mt-3 grid grid-cols-3 gap-3">{["Product profile", "Seal detail", "Media construction"].map((label, index) => <div key={label} className="relative aspect-square overflow-hidden rounded-md border border-[#353d43] bg-[#171c20]"><FilterVisual category={product.category} compact imageSrc={product.images?.[index] ?? product.image} /><span className="absolute inset-x-0 bottom-0 border-t border-[#353d43] bg-[#0d1114]/95 px-2 py-1.5 text-center font-mono text-[8px] font-bold uppercase tracking-[0.06em] text-[#c9cdd0]">0{index + 1} · {label}</span></div>)}</div></div>
