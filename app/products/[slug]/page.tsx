  ...
  return (
    <>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-5 pb-16 sm:px-8 lg:grid-cols-[1.02fr_0.98fr] lg:px-10 lg:pb-20">
        <div><div className="aspect-[4/3] overflow-hidden rounded-[28px] border border-[#dce3ea] bg-[#f3f0e8] shadow-[0_18px_50px_rgba(7,23,43,0.08)]"><FilterVisual category={product.category} imageSrc={product.image} /></div><div className="mt-3 grid grid-cols-3 gap-3">{["Product profile", "Seal detail", "Media construction"].map((label, index) => <div key={label} className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-[#dce3ea] bg-[#f3f0e8]"><FilterVisual category={product.category} compact imageSrc={product.images?.[index] ?? product.image} /><span className="absolute inset-x-0 bottom-0 bg-white/95 px-2 py-1.5 text-center text-[9px] font-bold text-[#48566a]">0{index + 1} · {label}</span></div>)}</div></div>
