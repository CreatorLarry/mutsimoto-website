    ...
    <>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-2">
        {downloads.map((download) => <article key={download.id} className="rounded-[22px] border border-[#e0e6ed] bg-white p-5 shadow-[0_8px_28px_rgba(7,23,43,0.04)] sm:p-6">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 border-y border-[#edf0f3] py-4 text-xs font-bold text-[#667387]"><span>{fileSize(download.fileSize)} PDF</span><span>Updated {new Date(download.updatedAt).toLocaleDateString("en-KE")}</span><a href={download.publicUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-[#d51f2a] hover:underline">Open file <ExternalLink className="size-3" /></a></div>
