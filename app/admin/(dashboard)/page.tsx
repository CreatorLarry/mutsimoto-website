  ...
  return (
    <>
      ...
      <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        <StatCard label="Total products" value={data.counts.totalProducts} icon={Boxes} href="/admin/products" />
        <StatCard label="Published" value={data.counts.publishedProducts} icon={CheckCircle2} tone="green" href="/admin/products?status=published" />
        <StatCard label="Drafts" value={data.counts.draftProducts} icon={FileEdit} href="/admin/products?status=draft" />
        <StatCard label="Under review" value={data.counts.reviewProducts} icon={ClipboardClock} tone="amber" href="/admin/products?status=review" />
        {canSeeEnquiries && <StatCard label="New enquiries" value={data.counts.newEnquiries} icon={MailQuestion} tone="red" href="/admin/enquiries?status=new" />}
        {canSeeEnquiries && <StatCard label="Awaiting follow-up" value={data.counts.followUpEnquiries} icon={RefreshCw} tone="amber" href="/admin/enquiries?status=follow_up" />}
      </section>
