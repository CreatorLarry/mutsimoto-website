...
export default function Home() {
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-[linear-gradient(120deg,#e9f4f1_0%,#f8f4ed_54%,#ffeae7_100%)]">
        ...
        <div className="relative mx-auto grid min-h-[690px] max-w-7xl items-center gap-12 px-5 py-14 sm:px-8 lg:grid-cols-[1.03fr_0.97fr] lg:px-10 lg:py-20">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full border border-[#f3ccc7] bg-[#fffaf4] px-4 py-2 text-[10px] font-extrabold uppercase tracking-[0.15em] text-[#d51f2a] shadow-sm"><span className="size-2 rounded-full bg-[#e52833]" /> Automotive &amp; industrial filtration</p>
            ...
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#647287]">Oil, fuel, and air filters engineered around the vehicles, machinery, and power systems that keep your operation moving.</p>
      ...
      <section className="bg-[#fffdf9]"><div className="mx-auto grid max-w-7xl grid-cols-2 gap-3 px-5 py-8 sm:px-8 md:grid-cols-4 lg:px-10">{[
        ["01", "Automotive range", "bg-[#fff0ec]"], ["02", "Industrial coverage", "bg-[#eaf4f1]"], ["03", "OEM references", "bg-[#fff4df]"], ["04", "Technical support", "bg-[#edf2f7]"],
      ...
      <section className="bg-[#eaf3f1]"><div className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-28"><div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><SectionHeading eyebrow="Browse by application" title="Start with the equipment you service" description="Choose an application, then narrow results by make, engine, equipment, or existing reference." /><ButtonLink href="/applications" variant="outline">All applications <ArrowRight className="size-4" /></ButtonLink></div><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{applications.map((application) => <ApplicationCard key={application.id} application={application} />)}</div></div></section>

      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10 lg:py-28"><div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><SectionHeading eyebrow="Featured products" title="Popular filtration references" description="Fast-moving automotive and industrial filters with clear fitment and technical data." /><Link href="/products" className="inline-flex items-center gap-2 text-sm font-bold text-[#07172b] transition-colors hover:text-[#d51f2a]">View full catalogue <ArrowRight className="size-4" /></Link></div><div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{featuredProducts.map((product) => <ProductCard key={product.id} product={product} />)}</div></section>

      <section className="px-5 py-8 sm:px-8 lg:px-10 lg:py-12"><div className="blueprint-grid-dark mx-auto grid max-w-7xl gap-12 overflow-hidden rounded-[32px] bg-[linear-gradient(120deg,#07172b,#123446)] px-6 py-14 text-white shadow-[0_24px_70px_rgba(7,23,43,0.18)] sm:px-10 lg:grid-cols-[0.82fr_1.18fr] lg:px-14 lg:py-20"><div><SectionHeading eyebrow="Why Mutsimoto" title="Filtration knowledge behind every fitment" description="A focused range, practical application guidance, and technical support for confident product selection." light /><ButtonLink href="/about" variant="secondary" className="mt-8">About Mutsimoto <ArrowRight className="size-4" /></ButtonLink></div><div className="grid gap-3 sm:grid-cols-2">{reasons.map(({ icon: Icon, title, text, tone }) => <article key={title} className="rounded-[20px] border border-white/10 bg-white/[0.055] p-6"><span className={`grid size-11 place-items-center rounded-2xl ${tone} text-white`}><Icon className="size-5" /></span><h3 className="mt-5 text-lg font-extrabold">{title}</h3><p className="mt-2 text-sm leading-6 text-[#aeb9c8]">{text}</p></article>)}</div></div></section>
      ...
      <section className="px-5 pb-4 sm:px-8 lg:px-10"><div className="mx-auto flex max-w-7xl flex-col gap-8 rounded-[28px] bg-[#e52833] px-6 py-10 text-white shadow-[0_20px_55px_rgba(229,40,51,0.18)] sm:px-10 lg:flex-row lg:items-center lg:justify-between lg:px-12"><div className="flex gap-5"><span className="hidden size-13 shrink-0 place-items-center rounded-2xl bg-white/15 sm:grid"><BookOpen /></span><div><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-white/70">Catalogue resources</p><h2 className="mt-2 text-3xl font-extrabold tracking-[-0.04em]">Take the filter range with you.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/80">Request the latest catalogue, cross-reference guide, or technical data sheet.</p></div></div><ButtonLink href="/downloads" variant="secondary">Browse downloads <ArrowRight className="size-4" /></ButtonLink></div></section>
