...
export default async function Home() {
  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-[#e7d5ca] bg-[linear-gradient(120deg,#d5ebe6_0%,#f8eddc_52%,#ffd9d5_100%)]">
        ...
        <div className="relative mx-auto grid min-h-[690px] max-w-7xl items-center gap-12 px-5 py-14 sm:px-8 lg:grid-cols-[1.03fr_0.97fr] lg:px-10 lg:py-20">
          ...
          <div className="relative">
            ...
            <div className="absolute -left-4 top-9 rounded-2xl border border-[#f4ded5] bg-[#fffaf4] px-4 py-3 shadow-[0_14px_35px_rgba(7,23,43,0.14)] sm:-left-8"><p className="text-[9px] font-extrabold uppercase tracking-[0.13em] text-[#526176]">Product focus</p><p className="mt-1 text-sm font-extrabold text-[#07172b]">Oil · Fuel · Air</p></div>
            <div className="absolute -bottom-5 right-5 rounded-2xl bg-[#c91923] px-5 py-4 text-white shadow-[0_14px_35px_rgba(229,40,51,0.28)]"><p className="text-[9px] font-bold uppercase tracking-[0.13em] text-white">Coverage</p><p className="mt-1 text-sm font-extrabold text-white">Auto + Industrial</p></div>
          </div>
        </div>
        <div className="relative mx-auto max-w-7xl px-5 pb-10 sm:px-8 lg:px-10">
          <div className="rounded-[24px] border border-[#eadfd4] bg-[#fffdf9] p-4 shadow-[0_18px_55px_rgba(7,23,43,0.1)] sm:p-5"><p className="mb-3 px-2 text-[10px] font-extrabold uppercase tracking-[0.15em] text-[#526176]">Find the right filter in seconds</p><SearchBar submitHref="/products" placeholder="Search by part number, OEM number, vehicle, engine, or equipment" /></div>
        </div>
      </section>
      ...
      <section className="px-5 pb-4 sm:px-8 lg:px-10"><div className="dark-panel mx-auto flex max-w-7xl flex-col gap-8 rounded-[28px] bg-[#b91821] px-6 py-10 text-white shadow-[0_20px_55px_rgba(229,40,51,0.22)] sm:px-10 lg:flex-row lg:items-center lg:justify-between lg:px-12"><div className="flex gap-5"><span className="hidden size-13 shrink-0 place-items-center rounded-2xl bg-white/15 sm:grid"><BookOpen /></span><div><p className="text-[10px] font-extrabold uppercase tracking-[0.15em] text-white">Catalogue resources</p><h2 className="mt-2 text-3xl font-extrabold tracking-[-0.04em] text-white">Take the filter range with you.</h2><p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-white">Request the latest catalogue, cross-reference guide, or technical data sheet.</p></div></div><ButtonLink href="/downloads" variant="secondary">Browse downloads <ArrowRight className="size-4" /></ButtonLink></div></section>
