    ...
    <>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[0.72fr_1.28fr] lg:px-10 lg:py-20">
        <aside>
          ...
          {nakuruBranch && <><div className="mt-8 space-y-5 border-y border-[#353d43] py-7 text-sm text-[#c9cdd0]"><a href={`tel:${nakuruBranch.phone.replace(/\s/g, "")}`} className="flex items-center gap-3 hover:text-white"><Phone className="size-5 text-[#ef3340]" />{nakuruBranch.phone}</a><a href={`mailto:${nakuruBranch.email}`} className="flex items-center gap-3 hover:text-white"><Mail className="size-5 text-[#ef3340]" />{nakuruBranch.email}</a><p className="flex items-start gap-3"><Clock3 className="mt-0.5 size-5 shrink-0 text-[#ef3340]" />{nakuruBranch.openingHours}</p></div><ButtonLink href={nakuruBranch.whatsappUrl} external variant="whatsapp" className="mt-7 w-full"><MessageCircle className="size-4" /> Contact us on WhatsApp</ButtonLink></>}
        </aside>
