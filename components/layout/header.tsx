      ...
      <div className="mx-auto flex h-[82px] max-w-[1480px] items-center justify-between gap-4 px-5 sm:px-8 lg:px-10 xl:px-6 2xl:px-8">
        ...
        <nav className="site-header__nav hidden shrink-0 items-center gap-0.5 rounded-md border border-[#2e363c] bg-[#171d22] p-1 xl:flex" aria-label="Main navigation">
          {navigation.map((item) => {
            ...
            return (
              <a key={item.href} href={item.href} className={cn("site-header__link whitespace-nowrap rounded px-2.5 py-2.5 text-[10px] font-extrabold uppercase tracking-[0.035em] text-[#aeb4b8] transition-all hover:bg-[#232a30] hover:text-white 2xl:px-3.5 2xl:text-[11px] 2xl:tracking-[0.04em]", active && "site-header__link--active bg-[#2a3035] text-white shadow-[inset_0_-2px_0_#ef3340]")}>{item.label}</a>
            );
          })}
        </nav>
        <div className="flex shrink-0 items-center gap-2">
          <Link href="/products" className="site-header__action grid size-11 place-items-center rounded-md border border-[#53565a] bg-[#171d22] text-white transition-all hover:border-[#ef3340] hover:bg-[#232a30] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ef3340]" aria-label="Search products"><Search className="size-4.5" /></Link>
          ...
          <button type="button" onClick={() => setMobileOpen((open) => !open)} className="site-header__action grid size-11 place-items-center rounded-md border border-[#53565a] bg-[#171d22] text-white xl:hidden" aria-label={mobileOpen ? "Close menu" : "Open menu"} aria-expanded={mobileOpen}>{mobileOpen ? <X className="size-5" /> : <Menu className="size-5" />}</button>
        </div>
      </div>
      {mobileOpen && (
        <div className="site-header__mobile border-t border-[#353d43] bg-[#0d1114] px-5 pb-6 shadow-[0_20px_35px_rgba(0,0,0,0.32)] xl:hidden">
          <nav className="mx-auto grid max-w-7xl gap-1 pt-4" aria-label="Mobile navigation">
            {navigation.map((item) => <a key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className={cn("site-header__mobile-link rounded px-4 py-3.5 text-sm font-bold text-[#b9bec2] hover:bg-[#1a2025] hover:text-white", pathname === item.href && "site-header__mobile-link--active border-l-2 border-[#ef3340] bg-[#1a2025] text-white")}>{item.label}</a>)}
