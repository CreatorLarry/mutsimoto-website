...
export function CatalogueExplorer({ products, initialQuery = "", initialCategory = "", initialApplication = "", initialEquipment = "" }: CatalogueExplorerProps) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-10 lg:py-16">
      <div className="rounded-[22px] border border-[#e0e6ed] bg-[#f5f7fa] p-4 shadow-[0_8px_30px_rgba(7,23,43,0.035)] sm:p-5"><SearchBar value={query} onValueChange={setQuery} placeholder="Search by part number, OEM number, vehicle, engine, or equipment" /></div>
      ...
      <div className="mt-7 grid gap-8 lg:grid-cols-[250px_1fr]">
        <FilterSidebar className="hidden rounded-[22px] border border-[#e0e6ed] bg-[#f8fafb] p-5 lg:block" filters={filters} options={options} onChange={setFilters} onReset={reset} />
